	 ...
	 var methods = {
		...
		*/
		_funcCall: function(field, rules, i, options) {
			// modif ppl gfit : aller chercher la fonction dans les data du field
			var functionName = rules[i + 1];
			var fn = $(field).data(functionName);
		...
		*/
		_future: function(form, field, rules, i, options) {
			// modifs par ppl gfit : utilisation de datepicker('getDate') si possible
			var p=rules[i + 1];
			var fieldAlt = $(form.find("input[name='" + p.replace(/^#+/, '') + "']"));
			var pdate;
			...
			if (p.toLowerCase() == "now") {
				...
				if (fieldAlt.is(":disabled"))
					return;
				else if(fieldAlt.hasClass('hasDatepicker'))
					pdate = fieldAlt.datepicker('getDate');
				else
					pdate = methods._parseDate(fieldAlt.val());
			} else {
				pdate = methods._parseDate(p);
			...
			if(field.hasClass('hasDatepicker'))
				vdate = field.datepicker('getDate');
			else
